export default {
  ...
  methods: {
    loadItemSelection: function () {
      const id = this.value[this.itemType.itemKey]
    ...
    selectRandomPaintColor: function () {
      const randomIndex = Math.floor(Math.random() * this.paintColors.length)
  ...
  computed: {
    ...
    selectedPaintColorClass: function () {
      const color = this.paintColors.find(el => el.id === this.selectedPaint)
