...
export default {
  ...
  methods: {
    passesFilter: function (runnable) {
      ...
      if (!runnable.name.toLowerCase().includes(this.botNameFilter.toLowerCase())) { return false }

      if (runnable.runnable_type === 'human') { return this.displayHuman }

      // show psyonix bots only in specific categories
      if (runnable.runnable_type === 'psyonix') { return category.includePsyonixBots }
      ...
      if (runnable.runnable_type === 'script' && !category.displayScriptDependencies && runnable.enabled) { return true }

      const allowedTag = runnable.runnable_type === 'script' ? category.scripts : category.bots
      if (allowedTag) {
        if (allowedTag === '*') { return true }

        if (runnable.info == null) { return false }
