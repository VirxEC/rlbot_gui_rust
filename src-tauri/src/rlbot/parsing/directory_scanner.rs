...
pub async fn scan_directory_for_bot_configs(root_dir: &str) -> Vec<BotConfigBundle> {
    scan_directory_for_item(root_dir, BotConfigBundle::minimal_from_path).await
}
...
pub async fn scan_directory_for_script_configs(root_dir: &str) -> Vec<ScriptConfigBundle> {
    scan_directory_for_item(root_dir, ScriptConfigBundle::minimal_from_path).await
}
...
async fn scan_directory_for_item<T, R, F>(root_dir: &str, func: F) -> Vec<T>
where
    T: Sized,
