...
// This code was taken from the configparser crate with the indexmap feature enabled.
// The async load/write patch and multiline patches have been merged as well.
// Crate maintainer seems to now be inactive on the crate; this is the reason for this file existing.
// The original crate: https://crates.io/crates/configparser
// The project on github: https://github.com/QEDK/configparser-rs
// The merged patches: https://github.com/QEDK/configparser-rs/pull/28 && https://github.com/QEDK/configparser-rs/pull/30
// Default settings have been changed for RLBot, including the comment symbol being set to ';' only and multiline being on
...
use tokio::{fs as async_fs, fs::File as AsyncFile, io::AsyncReadExt};
...
pub struct IniDefault {
    ...
    ///let mut config = Ini::new();
    ///let default = config.defaults();
    ///assert_eq!(default.multiline, true);
    ///```
    pub multiline: bool,
}
...
impl Ini {
    ...
    fn unparse(&self) -> String {
        ...
        fn unparse_key_values(out: &mut String, outmap: &Map<String, Option<String>>, multiline: bool) {
    ...
    fn parse(&self, input: String) -> Result<Map<String, Map<String, Option<String>>>, String> {
        ...
        for (num, line) in input.lines().enumerate() {
            ...
            match (trimmed.find('['), trimmed.rfind(']')) {
                ...
                (Some(0), None) => {
                    return Err(format!("line {}: Found opening bracket for section name but no closing bracket", num));
                }
                _ => {}
            }
            ...
            if line.starts_with(char::is_whitespace) && self.multiline {
                let key = match current_key.as_ref() {
                    Some(x) => x,
                    None => return Err(format!("line {} started with identation but there is no current entry", num,)),
                };

                let valmap = map.entry(section.clone()).or_insert_with(Map::new);

                let val = valmap.entry(key.clone()).or_insert_with(|| Some(String::new()));
    ...
    pub fn remove_section(&mut self, section: &str) -> Option<Map<String, Option<String>>> {
        let section = if self.case_sensitive { section.to_owned() } else { section.to_lowercase() };
